...
Does an ontology actually help you choose training data — or does the *shape* of
the selection algorithm do all the work?

This repository contains the full pipeline, results, and paper for a registered
report testing whether **MeSH concept-coverage selection** beats uniform sampling
when fine-tuning a medical instruction model at a fixed token budget. Selection
runs on CPU only, so the premise is a cost asymmetry: embedding-based selectors
(D4 and its descendants) must push the whole candidate pool through a GPU encoder
before a single training step; this one does not.

The pre-registered mechanism gate **failed**. That is the finding, and the design
was built so a failure would identify its own cause.
...
- **What is actually operative:** the per-token objective
  ...
  examples at a fixed 500-step budget means more distinct instructions per
  optimiser step. No ontology required.
...
2. **The cost asymmetry is real and larger than expected.** Tagging + selection
   over the 167k pool cost **42 CPU-seconds and zero GPU-seconds**. Encoding the
   same 73.9M tokens with a 22.7M-param MiniLM is 3.35e15 FLOPs at 2P'T — **36%
   of the 9.24e15 FLOPs of the LoRA fine-tune it is meant to make cheaper**.

Full argument, figures, and limitations: `paper.latex`.
...
1. **Lexical grounding** — Aho–Corasick over MeSH 2026 labels and entry terms
   ...
   chars and common English words are dropped (0.1% discard rate).
...
only for subset selection**, and the training data stays unmodified natural
language.
...
| `length` | longest first | none | controls for "just picks long docs" |
| `ontology` | coverage per token | CPU only | the method |
| `placebo` | **permuted tags**, same algorithm | CPU only | mechanism control |
| `d4` | encode → kmeans → semdedup → sample | 1 GPU pass | the real competitor |
...
beam_app.py               GPU stages on Beam (H100/A10G) — the runner actually used
...
HANDOFF.md                current state, corrections, and what was never run
PLAN.md                   what remains, and the decision table that got us here
KAGGLE_NOTEBOOK.md        historical — the original Kaggle plan, superseded by Beam
```
...
MedExQA), `results/tags/tags.jsonl{,.gz}`, `results/tags/concepts.json`, the
`overleaf*/` build directories and zips.
...
`tests/smoke_test.py` runs on a synthetic MeSH fragment and needs no data. It
...
### The one invariant that will silently ruin everything
...
machine ever see different pool files, every arm is quietly garbage with no
error raised. `pool.jsonl` is built **once**, sha256-hashed, and never
regenerated; the hash is recorded in every selection output. If it must be
rebuilt, all tags and selections die with it.
...
ran on **Beam** (`beam_app.py`), which superseded an earlier Kaggle-T4 plan
(`KAGGLE_NOTEBOOK.md`, kept for history) and a Modal attempt (`modal_app.py`).
...
Each reported run is ~9 minutes of training. Note that T4 is Turing, so the code
uses `fp16` not `bf16` and `sdpa` not flash-attn-2 — harmless on newer cards.
...
rewrites the paths, and zips it — upload `overleaf_flat.zip` to Overleaf as-is.
It errors loudly if a referenced figure is missing. Both `overleaf_flat/` and the
zips are build outputs and are gitignored; regenerate rather than edit them.

`09_write_results.py` is idempotent: it rewrites everything between the
`% BEGIN AUTOGEN training-comparisons` / `% END AUTOGEN` markers in
...
Complete and defensible as a registered report with a diagnosed null. Not done:
the `d4` arm (repeated container SIGSEGVs on the available GPU tier — reported
analytically in FLOPs, not wall-clock), the full budget sweep, the five
ablations, and `02_validate_tagger.py` (tagger P/R against BioASQ). See
`PLAN.md` for what each would take and `HANDOFF.md` for why the ablations now
look like the wrong questions.
